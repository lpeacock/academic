...
public class LinkedSet<T extends Comparable<? super T>> implements Set<T> {
   ...
   public Set<T> union(LinkedSet<T> s) {
      ...
      LinkedSet<T> output = new LinkedSet<T>();
      output.front = this.front;
      output.rear = this.rear;
      Node current = output.front;
      Node currentS = s.front;
      int counter = 0;
   
      while (currentS != null) {
      
         if (currentS.element.compareTo(current.element) > 0 && current.next != null && currentS.element.compareTo(current.next.element) < 0) {
            currentS.next = current.next;
            current.next = currentS.next;
            current.next.prev = current;
            counter++;
         }
         // if (currentS.element.compareTo(current.element) < 0) {
         // currentS.next = 
         // }
         else if (current == this.rear && currentS.element.compareTo(this.rear.element) > 0) {
            currentS.prev = output.rear;
            output.rear.next = currentS;
            output.rear = currentS;
            counter++;
         
         }
         
         else if (current == this.front && currentS.element.compareTo(this.front.element) < 0) {
            currentS.next = output.front;
            output.front.prev = currentS;
            output.front = currentS;
            counter++;
   ...
   public Iterator<Set<T>> powerSetIterator() {
      Iterator<Set<T>> it = 
         new Iterator<Set<T>>() {
            private Node current = front;
            private String size = Integer.toString(size());
            private LinkedSet<T> a = new LinkedSet<T>();
            private int counter = 0;
            private String binary = Integer.toBinaryString(counter);
            private String binaryCounter = String.format("%04d", binary);
